...
const UrlBuilder = () => {
  ...
  const setConfigBaseUrl = () => {
    _configurationsUrl = getBaseUrl() + '/collections'
  }
   ...
   * @returns {String} url
   */
  const getConfigUrl = (collectionId, environmentId) => {
    return _configurationsUrl + `/${collectionId}/config?environment_id=${environmentId}`
  }
  ...
  const getBaseUrl = () => {
    if (_overrideServerHost) {
      return _overrideServerHost + _service + _path + _instanceGuid;
    } else {
  ...
  const getMeteringurl = (instanceGuid) => {
    ...
    if (_overrideServerHost) {
      base = _overrideServerHost + _service;
    }
