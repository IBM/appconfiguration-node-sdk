...
const Metering = () => {
    ...
    function createInstance() {
         ...
         */
        async function addMetering(guid, collectionId, identityId, segmentId, featureId, propertyId) {
            writingThread += 1
            ...
            if (meteringData.hasOwnProperty(guid)) {
                if (meteringData[guid].hasOwnProperty(collectionId)) {
                    if (meteringData[guid][collectionId].hasOwnProperty(modifyKey)) {
                        if (meteringData[guid][collectionId][modifyKey].hasOwnProperty(identityId)) {
                            if (meteringData[guid][collectionId][modifyKey][identityId].hasOwnProperty(segmentId)) {
                                hasData = true;
                                meteringData[guid][collectionId][modifyKey][identityId][segmentId].evaluation_time = time
                                meteringData[guid][collectionId][modifyKey][identityId][segmentId].count += 1
                            }
                        } else {
                            meteringData[guid][collectionId][modifyKey][identityId] = {}
                            meteringData[guid][collectionId][modifyKey][identityId][segmentId] = {}
                        }
                    } else {
                        meteringData[guid][collectionId][modifyKey] = {}
                        meteringData[guid][collectionId][modifyKey][identityId] = {}
                        meteringData[guid][collectionId][modifyKey][identityId][segmentId] = {}
                    }
                } else {
        ...
        function buildRequestBody(sendMeteringData, result, key) {
            Object.keys(sendMeteringData).forEach(guid => {
                ...
                Object.keys(sendMeteringData[guid]).forEach(collectionId => {
                    let collections = {
                        "collection_id": collectionId,
                        "usages": []
                    }
                    let features = sendMeteringData[guid][collectionId];
                    Object.keys(features).forEach(featureId => {
                        Object.keys(features[featureId]).forEach(identityId => {
                            Object.keys(features[featureId][identityId]).forEach(segmentId => {
                                let usages = {
                                    [key]: featureId,
                                    "identity_id": identityId,
                                    "segment_id": segmentId === Constants.DEFAULT_SEGMENT_ID ? null : segmentId,
                                    "evaluation_time": features[featureId][identityId][segmentId]["evaluation_time"],
                                    "count": features[featureId][identityId][segmentId]["count"]
                                }
