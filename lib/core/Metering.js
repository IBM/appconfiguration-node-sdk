...
const Metering = () => {
    ...
    function createInstance() {
         ...
         */
        async function addMetering(guid, environmentId, collectionId, identityId, segmentId, featureId, propertyId) {
            writingThread += 1
                ...
                if (meteringData[guid].hasOwnProperty(environmentId)) {
                    if (meteringData[guid][environmentId].hasOwnProperty(collectionId)) {
                        if (meteringData[guid][environmentId][collectionId].hasOwnProperty(modifyKey)) {
                            if (meteringData[guid][environmentId][collectionId][modifyKey].hasOwnProperty(identityId)) {
                                if (meteringData[guid][environmentId][collectionId][modifyKey][identityId].hasOwnProperty(segmentId)) {
                                    hasData = true;
                                    meteringData[guid][environmentId][collectionId][modifyKey][identityId][segmentId].evaluation_time = time
                                    meteringData[guid][environmentId][collectionId][modifyKey][identityId][segmentId].count += 1
                                }
                            } else {
                                meteringData[guid][environmentId][collectionId][modifyKey][identityId] = {}
                                meteringData[guid][environmentId][collectionId][modifyKey][identityId][segmentId] = {}
                            }
                        } else {
