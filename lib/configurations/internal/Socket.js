...
const { getHeaders, getRelayProxyConfigured, getToken } = require('../../core/ApiManager');
...
const watchdogState = {
  ...
  checkCount: 0             // used to occasionally log the watchdog
};


// Watchdog health check - runs every 15 minutes to detect if watchdog stopped unexpectedly
let healthCheckInterval = null;

function startWatchdogHealthCheck() {
  if (healthCheckInterval) {
    clearInterval(healthCheckInterval);
  }

  healthCheckInterval = setInterval(() => {
    // If we think we're connected but watchdog isn't running, restart it
    logger.log(`_isActive :: ${_isActive} :watchdogState.isEnabled:: ${watchdogState.isEnabled} :watchdogState.intervalId:: ${watchdogState.intervalId}`);
    if (_isActive && !watchdogState.isEnabled) {
      logger.warning('CRITICAL: Watchdog should be running but is not - restarting immediately');
      startWatchdog();
    }
...
async function scheduleRetry(eventToEmit, eventData, logMsg) {
  ...
  retryState.retryTimeoutId = setTimeout(() => {
    ...
    logger.log(`Emitted event ${eventToEmit} after ${delay}ms to trigger websocket reconnect`);
  }, delay);
}
...
function startWatchdog() {
  ...
  watchdogState.intervalId = setInterval(() => {
    ...
    // Log every 10 minutes (since checkInterval is 60s)
    if (watchdogState.checkCount % 10 === 0) {
      logger.log(`Watchdog active check - time since last heartbeat: ${(timeSinceLastHeartbeat / 1000).toFixed(1)}s`);
    }
...
function stopWatchdog() {
  if (watchdogState.intervalId) {
    logger.log('Stopping Watchdog timer');
    clearInterval(watchdogState.intervalId);
    watchdogState.intervalId = null;
  } else {
    logger.log('Watchdog is not running');
  }
...
socketClient.on('connect', (connection) => {
  ...
  connection.on('close', async (result, description) => {
    logger.log(`On connection close`);
    _isActive = false;
    stopWatchdog();
    if (result === Constants.CUSTOM_SOCKET_CLOSE_REASON_CODE) {
      resetRetryState();
      logger.log(`Websocket connection closed by the client. Reason: ${result} ${description}`);
    } else {
...
async function connectWebSocket() {
  try {
    logger.log('Initiating websocket connection to the App Configuration server...');
    if (socketConnectComplete === false) {
      logger.log('Websocket connection is already in progress, skipping duplicate connect attempt');
      return;
    }
    socketConnectComplete = false;
    const urlForWebsocket = urlBuilder.getWebSocketUrl();
    const headers = getHeaders();
    if (!getRelayProxyConfigured()) {
      headers.Authorization = await getToken();
    }
    logger.log('Successfully obtained authentication token for websocket connection');
    closeWebSocket(); // close existing websocket connection if any
    socketClient.connect(
      urlForWebsocket, [], [], headers,
    );
    logger.log('Websocket connection request sent to the App Configuration server');
  } catch (e) {
