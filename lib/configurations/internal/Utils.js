...
/**
 * Parse progressive rollout phases into B-Tree for timestamp-to-percentage lookups.
 * @param {Object} configuration - Rollout config with start_at and phases
 * @returns {BTree} B-Tree mapping timestamp (ms) -> percentage
 */
function parseRolloutConfigurationPhases(configuration) {
  if (!configuration?.start_at || !Array.isArray(configuration.phases)) {
    throw new Error('Invalid rollout configuration');
  }

  const multipliers = { 'days': 86400000, 'hours': 3600000, 'minutes': 60000 };
  const startTimestamp = Date.parse(configuration.start_at);

  if (Number.isNaN(startTimestamp)) {
    throw new Error(`Invalid start_at: ${configuration.start_at}`);
  }

  const result = new BTree();
  result.set(0, 0);
  let transitionTime = startTimestamp;

  configuration.phases.forEach((phase) => {
    if (typeof phase?.percentage === 'number') {
...
module.exports = {
  ...
  computeCapDelayMs,
  computeNextDelayMs,
  parseRolloutConfigurationPhases
};
