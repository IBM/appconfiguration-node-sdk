 ...
 * 
 * @returns {String} configurations from the given file path or empty string.
 */
function readPersistentCacheConfigurations(filePath) {
  ...
  try {
    ...
    data = fs.readFileSync(filePath, "utf-8").toString().trim();
    if (!data) {
 ...
 * 
 * @returns {String} bootstrap configurations from the given file path, if there was no error reading it.
 * @throws {Error} If filePath doesn't exist or is empty or fails to parse the json.
 */
