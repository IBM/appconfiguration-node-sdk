...
class SegmentRules {
  ...
  constructor(segmentRules) {
    ...
    this.rule_id = segmentRules.rule_id;
    this.value = segmentRules.value;
    this.order = segmentRules.order;
    this.rollout_type = Object.prototype.hasOwnProperty.call(segmentRules, 'rollout_type') ? segmentRules.rollout_type : Constants.MANUAL;
    if (segmentRules.rollout_configuration) {
      this.rollout_configuration = segmentRules.rollout_configuration;
    } else {
