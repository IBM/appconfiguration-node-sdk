...
class Feature {
  ...
  constructor(feature) {
    ...
    this.rollout_type = Object.prototype.hasOwnProperty.call(feature, 'rollout_type') ? feature.rollout_type : Constants.MANUAL;
    if (feature.rollout_configuration) {
      this.rollout_configuration = feature.rollout_configuration;
    } else {
  ...
  /**
   * Returns a json object containing evaluated value, enabled status & detailed reason.
   *
   ...
   *    // result.isEnabled or result['isEnabled']
   *    // result.details or result['details']
   * }
   * 
   * // Note: While experiment is running, the `true` value of result.isEnabled indicates that given entityId was part of experiment audience.
   * // The `false` value indicates the entityId was not part of experiment audience.
   * 
   * ```
   */
