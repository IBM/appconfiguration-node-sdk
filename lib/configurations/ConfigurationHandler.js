...
const { reportError, getNormalizedValue, extractConfigurations, formatConfig, parseRolloutConfigurationPhases } = require('./internal/Utils');
const { connectWebSocket, isConnected, startWatchdogHealthCheck } = require('./internal/Socket');
...
const { computeCapDelayMs, computeNextDelayMs } = require('./internal/Utils');
...
const ConfigurationHandler = () => {
  ...
  function createInstance() {
      ...
      if (data) {
        if (data.features) {
          ...
          features.forEach((feature) => {
            ...
            _featureMap[feature.feature_id] = featureObj
            // Parse feature-level progressive rollout
            if (featureObj.rollout_configuration) {
              _rolloutConfigMap[feature.feature_id] = parseRolloutConfigurationPhases(feature.rollout_configuration);
            }
            // Parse segment-level progressive rollout
            if (feature.segment_rules && Array.isArray(feature.segment_rules)) {
              feature.segment_rules.forEach((segmentRule) => {
                const segmentRuleObj = new SegmentRules(segmentRule);
                if (segmentRuleObj.rollout_configuration) {
                  const key = `${feature.feature_id}${Constants.DELIMITER}${segmentRule.rule_id}`;
                  _rolloutConfigMap[key] = parseRolloutConfigurationPhases(segmentRule.rollout_configuration);
                }
              });
            }
          });
    ...
    function stopBackgroundConfigRetry() {
      if (configRetry.timer) {
        clearTimeout(configRetry.timer);
        configRetry.timer = null;
      }
      if (configRetry.active) {
        logger.log('Background config retry stopped.');
      }
      configRetry.active = false;
      configRetry.attempt = 0;
      configRetry.capMs = null;
    }

    // Schedules the next background attempt
    function scheduleNextBackgroundAttempt(reason) {
      const delayMs = computeNextDelayMs(configRetry.attempt, configRetry.capMs);
      const delayMin = (delayMs / 60000).toFixed(2);
      if (configRetry.timer) clearTimeout(configRetry.timer);

      if (reason) {
        logger.warning(
          `${reason} Background config retry scheduled in ${delayMin} minutes (attempt #${configRetry.attempt + 1}).`
        );
      }

      configRetry.timer = setTimeout(async () => {
        const result = await fetchFromAPI({ background: true });

        if (result && result.ok) {
          // Success: fetchFromAPI already stopped the background retry.
          return;
        }

        if (!result || !result.retryable) {
          // Non-retryable (4xx except 429) or unexpected -> stop
          stopBackgroundConfigRetry();
          return;
        }

        // Retryable -> schedule next attempt with exponential backoff
        configRetry.attempt += 1;
        const nextReason = `Failed to fetch the configurations. ${result.status} ${result.message || ''}`;
        scheduleNextBackgroundAttempt(nextReason);
      }, delayMs);
    }


    // entry to start background retry
    function startBackgroundConfigRetry(reason) {
      if (configRetry.active) {
        logger.info(`Background config retry already active (attempt #${configRetry.attempt}). Reason: ${reason}`);
        return;
      }
      configRetry.active = true;
      configRetry.attempt = 0;
      configRetry.capMs = computeCapDelayMs();
      reason = `Starting background config retry (cap ${(configRetry.capMs / 3600000).toFixed(2)} hours). Reason: ${reason}`;
      scheduleNextBackgroundAttempt(reason);
    }

    // Build the request parameters once in a common function
    function buildConfigRequestParams() {
      return {
        options: {
    ...
    async function fetchFromAPI(opts = {}) {
      ...
      const parameters = buildConfigRequestParams();
      ...
      try {
        ...
        if (statusCode >= 400 && statusCode < 499 && statusCode !== 429) {
          ...
          logger.error(`${statusCode} ${errMsg} ${e.message ? e.message : e}`);
          return background
            ? { ok: false, retryable: false, status: statusCode, message: e.message || e }
            : undefined;
        }

        // Retryable (429, 5xx)
        if (background) {
          // In background mode, let the scheduler decide the next step
          return { ok: false, retryable: true, status: statusCode, message: e.message || e };
        }

        // Default behavior (non-background caller): start background retry loop
        startBackgroundConfigRetry(`Failed to fetch configs: ${statusCode} ${e.message || e}`);
        return;
      }

      // Success path: load, emit, persist
      ...
      // Stop any background retry because we have fresh config now
      stopBackgroundConfigRetry();

      return background ? { ok: true, status: _response.status } : undefined;
    }
    ...
    function beginWebsocketAndCheckInternet() {
      ...
      // start watchdog health check
      startWatchdogHealthCheck();
    }
      ...
      if (_liveUpdate) {
        ...
        try {
          ...
          if (_persistentCacheDirectory !== null && persistentCacheRead) {
            ...
            startBackgroundConfigRetry('liveUpdate initial failure with persistent cache fallback');
          ...
          if (_bootstrapFile !== null && !errorReadingBootstrapConfig) {
            ...
            logger.info(`${errMsg}. ${message}`);
            emitter.emit(Constants.APPCONFIGURATION_CLIENT_EMITTER, message);
            startBackgroundConfigRetry('liveUpdate initial failure with bootstrap fallback');
            beginWebsocketAndCheckInternet();
            return;
          }
          reportError(errMsg);
    ...
    async function socketActions() {
      // Not calling the fetchfromapi() immediately, instead added a random delay b/w 0-5s
      // to prevent thundering herd of "GET /config" requests when large number of clients are present
      const jitterMs = Math.floor(5000 * Math.random()); // 0–5000ms
      if (jitterMs > 0) {
        logger.info(`Config refresh will start in ${(jitterMs / 1000).toFixed(2)} seconds (jitter).`);
    ...
    function evaluateRules(_rulesMap, entityAttributes, feature, property, entityId) {
                ...
                for (let innerLevel = 0; innerLevel < segments.length; innerLevel += 1) {
                  ...
                  if (evaluateSegment(segmentKey, entityAttributes)) {
                    ...
                    if (feature !== null) {
                      ...
                      let segmentLevelRolloutPercentage = -1;
                      if (segmentRule.rollout_configuration || (segmentRule.rollout_type !== null && segmentRule.rollout_type === Constants.PROGRESSIVE)) {
                        let btree;
                        if (segmentRule.rollout_percentage === Constants.DEFAULT_ROLLOUT_PERCENTAGE) {
                          btree = _rolloutConfigMap[feature.feature_id];
                        } else {
                          btree = _rolloutConfigMap[`${feature.feature_id}${Constants.DELIMITER}${segmentRule.rule_id}`];
                        }
                        if (btree) {
                          entityId += segmentRule.rollout_configuration.start_at;
                          const entry = btree.getPairOrNextLower(Date.now());
                          segmentLevelRolloutPercentage = entry ? entry[1] : 0;
                        } else {
                          segmentLevelRolloutPercentage = 0;
                        }
                      } else {
                        segmentLevelRolloutPercentage = segmentRule.rollout_percentage === Constants.DEFAULT_ROLLOUT_PERCENTAGE ? feature.rollout_percentage : segmentRule.rollout_percentage;
                      }
                      ...
                      if (segmentLevelRolloutPercentage === 100 || (getNormalizedValue(''.concat(entityId, ':', feature.feature_id)) < segmentLevelRolloutPercentage)) {
                        ...
                        resultDict.details.reason = reasonSegmentValueFeatureFlag(segmentName, entityId, segmentLevelRolloutPercentage);
                        ...
                        resultDict.details.reason = reasonDisabledValueFeatureFlag_1(segmentName, entityId, segmentLevelRolloutPercentage);
                      }
                    } else {
      ...
      if (feature !== null) {
        ...
        if (feature.rollout_configuration) {
          const btree = _rolloutConfigMap[feature.feature_id];
          if (btree) {
            entityId += feature.rollout_configuration.start_at;
            const entry = btree.getPairOrNextLower(Date.now());
            rolloutPercentage = entry ? entry[1] : 0;
          } else {
            rolloutPercentage = 0;
          }
        } else {
          rolloutPercentage = feature.rollout_percentage !== undefined ? feature.rollout_percentage : 100;
        }
        if (rolloutPercentage === 100 || (getNormalizedValue(''.concat(entityId, ':', feature.feature_id)) < rolloutPercentage)) {
          resultDict.value = feature.enabled_value;
          resultDict.isEnabled = true;
          resultDict.details.valueType = `ENABLED_VALUE`;
          resultDict.details.rolloutPercentageApplied = true;
          resultDict.details.reason = reasonEnabledValueFeatureFlag_1(entityId, rolloutPercentage);
        } else {
          resultDict.value = feature.disabled_value;
          resultDict.isEnabled = false;
          resultDict.details.valueType = `DISABLED_VALUE`;
          resultDict.details.rolloutPercentageApplied = false;
          resultDict.details.reason = reasonDisabledValueFeatureFlag_2(entityId, rolloutPercentage);
        }
      } else {
        // evaluateRules was called for property
        resultDict.value = property.value;
    ...
    function featureEvaluation(feature, entityId, entityAttributes) {
      ...
      try {
        ...
        if (feature.enabled) {
          ...
          resultDict.details.valueType = `DISABLED_VALUE`;
          resultDict.details.rolloutPercentageApplied = false;
          resultDict.details.reason = reasonDisabledValueFeatureFlag_3(entityId, rolloutPercentage);
    ...
    emitter.on(Constants.SOCKET_CONNECTION_ERROR, () => {
      logger.log('Websocket connection to App Configuration server failed. Attempting to reconnect...');
      _onSocketRetry = true;
      connectWebSocket();
    });

    emitter.on(Constants.SOCKET_LOST_ERROR, () => {
      logger.log('Websocket connection to App Configuration server was lost. Attempting to reconnect...');
      _onSocketRetry = true;
      connectWebSocket();
    });

    emitter.on(Constants.SOCKET_CONNECTION_CLOSE, () => {
      logger.log('Websocket connection to App Configuration server was closed. Attempting to reconnect...');
      _onSocketRetry = true;
      connectWebSocket();
    });
